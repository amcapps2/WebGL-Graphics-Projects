...
/** @global Maximum Z value */
var maxZ;
/** @global Minimum Z value */
var minZ;


/** @global Quat params */
var quat_vec = glMatrix.quat.create();
var roll_Deg = 0;
var pitch_Deg = 0;
var speed = 0.001;

/** @global A glmatrix vector to use for transformations */
var tf_Vec = glMatrix.vec3.fromValues(0.0,0.0,-2.0);    

// View parameters
/** @global Location of the camera in world coordinates */
var eyePt = glMatrix.vec3.fromValues(0.0,0.0,-0.5);
...
function handleKeyDown(event) {
  ...
  if (event.key == "=") {
    ...
    speed -= 0.001;
  } else if (event.key == "Escape") {KeyboardEvent
      resetViewParameters();
  }
}
...
function resetViewParameters() {
 eyePt = glMatrix.vec3.fromValues(0.0,0.0,-0.5);
 viewDir = glMatrix.vec3.fromValues(0.0,0.0,-1.0);
 up = glMatrix.vec3.fromValues(0.0,1.0,0.0);
 viewPt = glMatrix.vec3.fromValues(0.0,0.0,0.0);
}
...
function startup() {
  ...
  myTerrain = new Terrain(120, -1, 1, -1, 1);
    //myTerrain = new Terrain(120,-0.5,0.5,-0.5,0.5);
  ...
  gl.clearColor(0.73, 0.90, 0.84,1.0);
    //0.82, 0.93, 0.90, 1.0
...
function setupShaders() {
  ...
  shaderProgram.locations.fogTrue = gl.getUniformLocation(shaderProgram, "fogTrue");
  shaderProgram.locations.c_time = gl.getUniformLocation(shaderProgram, "c_time");
}
...
function draw() {
  ...
  var tf_Vec = glMatrix.vec3.create();
  ...
  // Generate view matrix using lookat.
  ...
  // Roll 
    if (currentlyPressedKeys["ArrowLeft"]) {
      roll_Deg -= 1;
      glMatrix.quat.setAxisAngle(quat_vec, viewDir, degToRad(-1));
      glMatrix.vec3.transformQuat(viewDir, viewDir, quat_vec);
      glMatrix.vec3.transformQuat(up, up, quat_vec);
    ...
    if (currentlyPressedKeys["ArrowUp"]){
      pitch_Deg += 1;
      glMatrix.vec3.cross(cross, viewDir, up);
      glMatrix.quat.setAxisAngle(quat_vec, cross, degToRad(1));
      glMatrix.vec3.transformQuat(viewDir, viewDir, quat_vec);
      glMatrix.vec3.transformQuat(up, up, quat_vec);
    ...
    // Update display
    document.getElementById("roll").value = roll_Deg;
    document.getElementById("pitch").value = pitch_Deg;
    document.getElementById("speed").value = speed;
    
    // Move eye point
    movement = glMatrix.vec3.create();
    glMatrix.vec3.scale(movement, viewDir, speed);
    glMatrix.vec3.add(eyePt, eyePt, movement);
    
  // Create a lookat point in the direction of -z  
  glMatrix.vec3.add(lookAtPt, eyePt, viewDir);
    
  // Generate lookat matrix, then adjust modelViewMatrix
  ...
  //Transform the view
  var tfv = glMatrix.vec3.create();
  glMatrix.vec3.set(tfv, 0.0, -0.25, -2.0);
  glMatrix.mat4.translate(modelViewMatrix, modelViewMatrix, tfv);
  //glMatrix.mat4.rotateY(modelViewMatrix, modelViewMatrix, degToRad(rotDelta));
  glMatrix.mat4.rotateX(modelViewMatrix, modelViewMatrix, degToRad(-90));
  ...
  // Determine whether to add fog. 
  if (document.getElementById("fog").checked) {
    gl.uniform1i(shaderProgram.locations.fogTrue, true);
  } else {
    gl.uniform1i(shaderProgram.locations.fogTrue, false);
  }
 ...
 */
 function animate(currentTime) {
  ...
  //Adjust rotation
  rotDelta += 0.15;
  gl.uniform1f(shaderProgram.locations.c_time, currentTime);
  draw();
